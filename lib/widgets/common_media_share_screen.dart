    ...
    return GetBuilder<MediaShareController>(builder: (_) {
      return DirectionalityRtl(
        ...
        SingleChildScrollView(
          ...
          Divider(height: 1, thickness: 1, color: appCtrl.appTheme.borderColor),
          const VSpace(Sizes.s20),
          mediaCtrl.selectedIndex == 0
              ? mediaCtrl.shardMediaList.isNotEmpty
                  ? GridView.builder(
                      shrinkWrap: true,
                      ...
                      itemBuilder: (context, index) {
                        log("mediaCtrl.shardMediaList::${mediaCtrl.mediaList[1]}");
                        return ClipRRect(
                            borderRadius: const BorderRadius.all(
                                Radius.circular(AppRadius.r8)),
                            child: mediaCtrl.shardMediaList[index]["type"] ==
                                    MessageType.image.name
                                ? Image.network(decryptMessage(mediaCtrl.shardMediaList[index]["content"]),
                                        height: Sizes.s130,
                                        width: Sizes.s100,
                                        fit: BoxFit.fill)
                                    .inkWell(
                                ...
                                : ChatVideo(snapshot: decryptMessage(mediaCtrl.shardMediaList[index]["content"])));
                      })
                  : Column(children: [
              ...
              : mediaCtrl.selectedIndex == 1
                  ? mediaCtrl.shardDocList.isNotEmpty
                      ? Column(
                          children:
                              mediaCtrl.shardDocList.asMap().entries.map((e) {
                                log("mediaCtrl.shardDocList:::${mediaCtrl.shardDocList }");
                          return Column(
                                ...
                                if (e.value["type"] == MessageType.doc.name)
                                  decryptMessage(e.value["content"])
                                      ...
                                      ? SharedMediaLayout(
                                          ...
                                          value: e.value,
                                          onTap: () =>
                                              mediaCtrl.onTapDocs(e.value))
                                      : (decryptMessage(e.value["content"])
                                                  .contains(".xlsx") ||
                                              decryptMessage(e.value["content"])
                                                  .contains(".xls"))
                                          ? SharedMediaLayout(
                                              icon: eSvgAssets.excel,
                                              value: e.value,
                                              onTap: () =>
                                                  mediaCtrl.onTapDocs(e.value))
                                          : (decryptMessage(e.value["content"])
                      ...
                      : Column(children: [
                          Image.asset(eImageAssets.itemNotFound,
                              width: Sizes.s240),
                          const VSpace(Sizes.s10),
                          Text("No Item Found",
                              style: AppCss.manropeBold16
                  ...
                  : mediaCtrl.sharedLink.isNotEmpty || mediaCtrl.index==2
                      ? Column(
                          children: mediaCtrl.sharedLink
                              .asMap()
                              .entries
                              .map((e) {
                                log("mediaCtrl.selectedIndex ::${mediaCtrl.selectedIndex }");
                                return Column(
                                    mainAxisAlignment:
        ...
        if (mediaCtrl.isLoading) const CommonLoader()
      ]))));
    });
  }
}
