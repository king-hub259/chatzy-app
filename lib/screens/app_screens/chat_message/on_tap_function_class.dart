...
class OnTapFunctionCall {
  ...
  //on emoji select
  onEmojiSelect(ChatController chatCtrl, docId, emoji, title,em) async {
    ...
    newEmojiList = chatCtrl.localMessage[index].message![messageIndex].emojiList ?? [];
    if(newEmojiList.isNotEmpty){
      int isContain = newEmojiList.indexWhere((element) => element['uid'] == appCtrl.user['id']);
      if(isContain >=0){
        newEmojiList[isContain]['emoji'] = emoji;
      }else {
        newEmojiList.add(data);
      }
    }else{
      newEmojiList = [data];
    }

    log("newEmojiList ;$newEmojiList");

    chatCtrl.localMessage[index].message![messageIndex].emojiList = newEmojiList;
    chatCtrl.update();
    ...
    await FirebaseFirestore.instance
        ...
        .collection(collectionName.chat).get().then((v)async {
          v.docs.reversed.toList().asMap().entries.forEach((d)async {
            if(d.key == messageIndex){
              await FirebaseFirestore.instance
                  .collection(collectionName.users)
                  .doc(chatCtrl.pId)
                  .collection(collectionName.messages)
  ...
  onEmojiRemove(ChatController chatCtrl, docId, emoji, title,em) async {
    ...
    int messageIndex = chatCtrl.localMessage[index].message!
        .indexWhere((element) => element.docId == docId);
    log("indexindex:$messageIndex");
    chatCtrl.localMessage[index].message![messageIndex].emojiList!.removeWhere((element) => element['uid'] == appCtrl.user["id"],);
    chatCtrl.update();
    await FirebaseFirestore.instance
        .collection(collectionName.users)
        .doc(appCtrl.user["id"])
        ...
        .collection(collectionName.chat)
        .doc(docId)
        .update({"emojiList": chatCtrl.localMessage[index].message![messageIndex].emojiList});
    await FirebaseFirestore.instance
        .collection(collectionName.users)
        .doc(chatCtrl.pId)
        .collection(collectionName.messages)
  ...
  //catalogue tap
  // catalogueTap(ChatController chatCtrl, docId, MessageModel? document) {
  //   if (chatCtrl.selectedIndexId.isNotEmpty || chatCtrl.enableReactionPopup) {
  //     if (chatCtrl.selectedIndexId.isNotEmpty) {
  //       chatCtrl.enableReactionPopup = false;
  //       chatCtrl.showPopUp = false;
  ...
  //   } else {
  //     log("DECR MSG ${decryptMessage(document!.content)}");
  //     List<dynamic> listMap = jsonDecode(decrypt(document.content));
  //     log("jhgdf :$listMap");
  //     Get.to(CartScreen(cartList: listMap,document: document));
  //   }
  // }
