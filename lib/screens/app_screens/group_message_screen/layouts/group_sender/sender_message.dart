...
class GroupSenderMessage extends StatefulWidget {
  ...
  final int? index;
  final String? currentUserId, docId, title,senderName;


  const GroupSenderMessage(
      {super.key, this.document, this.index, this.currentUserId, this.docId, this.title, this.senderName})
  ;
...
class _GroupSenderMessageState extends State<GroupSenderMessage> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<GroupChatMessageController>(builder: (chatCtrl) {
      log("widget.document!.type::${widget.senderName}");
      return Stack(
                  ...
                  children: <Widget>[
                    Row(mainAxisAlignment: MainAxisAlignment.end,
                        children: <Widget>[
                          ...
                          if (widget.document!.type == MessageType.emoji.name)
                          // Text
                            GroupContent(
                                userList: chatCtrl.pData["groupData"]["users"],
                                isSearch: chatCtrl.searchChatId.contains(
                            ...
                            (decryptMessage(widget.document!.content)
                                ...
                                onTap: () =>
                                    GroupOnTapFunctionCall().docTap(
                                        ...
                                        widget.document))
                                : (decryptMessage(widget.document!.content)
                                .contains(".xlsx") || decryptMessage(widget
                                .document!.content)
                                .contains(".xls"))
                                ? ExcelLayout(
                              currentUserId: widget.currentUserId,
